#include "output_libnet.hpp"

#include <netinet/udp.h>
#include <netinet/tcp.h>
#include <netinet/ip.h>

OutputLibnet::OutputLibnet(): queue_(*(TpService::getInstance()->getIoService())) {
    l_ = libnet_init(LIBNET_RAW4, "",errbuf_);
}

OutputLibnet::~OutputLibnet() {
    libnet_destroy(l_);
}
    
bool OutputLibnet::init() {
    queue_.asyncPop(std::bind(&OutputLibnet::popHandler, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
    return true;
}

void OutputLibnet::handlePacket(std::shared_ptr<ParentPacket> parPkt) {
    PacketPtr pkt = std::dynamic_pointer_cast<Packet>(parPkt);
    queue_.push(pkt);
}

void OutputLibnet::popHandler(const std::error_code& ec, const PacketPtr pkt) {
    if(ec == queue_error_code::stopped){
        std::cout << "OutputLibnet pophandler got AsyncQueue Stopped" << std::endl;
        return;
    }
    if(l_ == NULL){
...
#ifdef TRACE_LOG
        std::cout << "OutputLibnet dumping packet: " << std::endl;
        pkt->dump();
#endif
...
void OutputLibnet::dumpMem(unsigned char* p,int len) {
    printf("\t\t------------------------------------------------");
    for (int i = 0 ; i < len; i++ ) {
        if (i%16==0) {
            printf("\n\t\t");
